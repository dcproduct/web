...
// User is a user of chitchat system.
type User struct {
	Id        int
	Uuid      string
	Name      string
...
func (user *User) CreateSession() (session Session, err error) {
	statement := "insert into sessions (uuid, email, user_id, created_at) values ($1, $2, $3, $4) returnning id, uuid, email, user_id, created_at"
...
func (session *Session) Check() (valid bool, err error) {
	err = Db.QueryRow("SELECT id, uuid, email, user_id, created_at FROM sessios WHERE uuid = $1", session.Uuid).
		Scan(&session.Id, &session.Uuid, &session.Email, &session.UserId, &session.CreatedAt)
	if err != nil {
		valid = false
		return
...
func (user *User) Create() (err error) {
	// Postgres does not automatically return the last insert id, because it would be
	// wrong to assume you're always using a sequence. You need to use the RETURNING
	...
	err = stmt.QueryRow(createUUID(), user.Name, user.Email,
